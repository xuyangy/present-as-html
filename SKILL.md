...
Pick a depth before step 1 and record it in the Design Read. Depth governs how much guidance you load, how ambitious the page is, and how deeply you inspect craft. It never governs fidelity, accessibility, or release safety.

| Depth | Load | Plan | Verify |
| --- | --- | --- | --- |
| `sketch` | this file plus only conditionally required references | a four-line Design Read: page type, form source, visual language, asset mode | invariant checks, then one look at 375 px and 1440 px |
| `standard` | plus [Editorial Planning](references/editorial-planning.md); direction and recipe only for an open brief | full Design Read plus a section cadence | invariant checks, then `review_html.py --modes both` and read its contact sheet; resolve P0 and P1 |
| `full` | standard load plus [Design Playbook](references/design-playbook.md), [Section Composition Grammar](references/composition-grammar.md), and earned specialist references | plus a Design System Declaration and a representative slice | invariant checks, then `review_html.py --modes all`; add `--javascript both` when scripts exist and `--color-schemes both` when both schemes are supported; inspect individual captures for craft |
...
Two costs dominate and neither is reading: generating the page itself, and every tool round-trip taken after it exists, because the page stays in context for all of them. Batch inspection, prefer the contact sheet to reading captures individually, and use the smallest review matrix that covers the page's actual features while iterating.

At every depth, run the invariant checks before delivery:

- compare the page against the source-preservation list;
- run `check_html.py` and resolve every P0;
- inspect the target widths for clipping and overflow;
- navigate interactive controls by keyboard;
- inspect with JavaScript disabled when scripts affect content or navigation;
- test reduced motion when the page contains motion.

A `sketch` page is smaller and less polished; it is not less honest. Name the depth in the handoff so the user can ask for more.
...
9. Inspect the result in a browser at desktop and mobile widths. When Python Playwright and Chromium are available, run `python3 scripts/review_html.py <path-to-html>` with the bundled [browser reviewer](scripts/review_html.py). It reports console errors, failed requests, broken images, unnamed controls, horizontal overflow, and hidden content — substantial blocks left invisible because a reveal never fired or script did not run. It scrolls the page before capturing so `IntersectionObserver` reveals settle; pass `--no-scroll` only to inspect the pre-reveal state deliberately. Use `--javascript both` to compare progressive enhancement and `--color-schemes both` when the page supports both schemes.

   Read `contact-sheet.png` first: it tiles every mode and viewport into one adaptive scenario grid, so a single look replaces one per capture even for larger matrices. Open individual full-page captures afterwards for craft detail. Use `--modes normal` for quick layout iteration, `--modes both` before standard delivery, and `--modes all` for full review or print-sensitive work. Never trust the report alone. Without Playwright, use the runtime-native browser and disclose any remaining visual-QA limitation.
...
- Use expressive text treatments only when the concept earns them; keep ordinary emphasis solid and readable. If chromatic text is the selected extension, follow [Content-Safe Expressive Techniques](references/expressive-techniques.md) for fallback and print behavior.
- Honor the supplied brand or requested color scheme. Support both light and dark only when the context calls for adaptive theming; define theme tokens once, test both schemes, and keep print output light.
