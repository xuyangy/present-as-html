...
3. Inspect at approximately 375 px, 768 px, and 1440 px; include unusually long headings and URLs. When Python Playwright and Chromium are available, run `python3 scripts/review_html.py <path-to-html>`, read `contact-sheet.png` first, then open individual captures for craft detail and check `review-report.json`. Add `--javascript both` when scripts affect content or navigation and `--color-schemes both` when the page supports adaptive theming. Treat any `hiddenContent` entry as a P0 until proven deliberate: it means a substantial block stayed invisible after the page settled.
